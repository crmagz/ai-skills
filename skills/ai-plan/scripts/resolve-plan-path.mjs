...
function defaultPlanHome(args) {
  ...
  const agent = String(args.agent ?? process.env.AI_PLAN_AGENT ?? 'codex').toLowerCase();
  if (agent === 'codex') return path.join(homedir(), '.codex');
  throw new Error(`unsupported AI_PLAN_AGENT: ${agent}; use codex or set AI_PLAN_HOME`);
}

function buildFileName({jira, title}) {
  const jiraSlug = kebabCase(jira);
  const titleSlug = kebabCase(title || 'plan');

  if (!jiraSlug) return `${titleSlug}.plan.md`;
  if (!titleSlug || titleSlug === 'plan') return `${jiraSlug}.plan.md`;
  if (titleSlug.startsWith(jiraSlug)) return `${titleSlug}.plan.md`;
  return `${jiraSlug}-${titleSlug}.plan.md`;
}
