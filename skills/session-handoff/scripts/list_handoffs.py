...
Searches for handoff documents in .codex/handoffs/ and legacy .claude/handoffs/.
- Filename with date
...
from handoff_paths import handoff_dirs
...
def main():
    ...
    if not handoffs:
        print(f"No handoffs found in {project_path}/.codex/handoffs/ or .claude/handoffs/")
        print("\nTo create a handoff, run: python create_handoff.py [task-slug]")
        return

    print(f"Found {len(handoffs)} handoff(s) in Codex and legacy handoff directories\n")
    print("-" * 80)
