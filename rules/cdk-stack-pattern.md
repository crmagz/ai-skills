...
When using packages like `@your-org/api-gateway` or `@your-org/cdk` that provide construct functions, call them directly from the stack and configure via `ProjectEnvironment` props.
...
```typescript
props.containerRepositories?.forEach(repo => {
    ...
    Tags.of(ecrRepo).add('Platform', 'shared');
});
```
